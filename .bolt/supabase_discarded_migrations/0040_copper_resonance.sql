...
BEGIN
  ...
  -- Always insert activity record for any auth attempt
  INSERT INTO login_activity (
    ...
    CASE 
      WHEN NEW.last_sign_in_at IS NOT NULL AND OLD.last_sign_in_at IS DISTINCT FROM NEW.last_sign_in_at THEN 'success'
      ELSE 'failed'
    END
  );
...
CREATE TRIGGER on_auth_user_login
  ...
  FOR EACH ROW
  WHEN (
    -- Capture any auth attempt by checking raw_user_meta_data changes
