/*
  ...
  1. Tables
    - billing_plans: Stores plan configurations
    - billing_plans_pricing: Stores pricing for each plan
    - business_users: Maps users to businesses with roles
    - business_subscriptions: Tracks business subscriptions
    - businesses: Stores business profiles

  2. Functions
    - create_pending_business: Creates initial business profile on signup
    - get_billing_plans_with_pricing: Retrieves plans with pricing
    - check_user_role: Validates user roles
    - get_default_business: Gets user's primary business
...
BEGIN
    ...
    VALUES ('Standard Plan', '{"feature1": "value1"}'::jsonb, true)
    RETURNING id INTO plan_id;

    -- Insert the pricing options
    INSERT INTO billing_plans_pricing (plan_id, pricing_model, price)
    VALUES
        (plan_id, 'monthly', 25.00),
        (plan_id, 'yearly', 20.00);
END
$$;
...
-- Add triggers for updated_at column updates
CREATE TRIGGER update_billing_plans_updated_at
  BEFORE UPDATE ON billing_plans
  FOR EACH ROW
  EXECUTE FUNCTION update_updated_at();

CREATE TRIGGER update_billing_plans_pricing_updated_at
  BEFORE UPDATE ON billing_plans_pricing
  FOR EACH ROW
  EXECUTE FUNCTION update_updated_at();
